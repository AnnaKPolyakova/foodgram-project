...
function Ingredients() {
    ...
    const addIngredient = (e) => {
        if(nameIngredient.value && cantidad.value) {
            ...
            elem.id = `ing_${cur}`;
            elem.innerHTML = `<span> ${data.title} ${data.amount}${data.measure}</span> <span class="form__field-item-delete"></span>
                             <input id="nameIngredient_${cur}" name="nameIngredient_${cur}" type="hidden" value="${data.title}">
                             <input id="valueIngredient_${cur}" name="valueIngredient_${cur}" type="hidden" value="${data.amount}">
                             <input id="unitsIngredient_${cur}" name="unitsIngredient_${cur}" type="hidden" value="${data.measure}">`;
            cur++;
    ...
    const getValue = (e) => {
        const data = {
            title: nameIngredient.value,
            amount: cantidad.value,
            measure: cantidadVal.textContent
        };
